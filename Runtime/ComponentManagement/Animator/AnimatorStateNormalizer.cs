...
public class AnimatorStateNormalizer : MonoBehaviour
{
    ...
    public void SetAnimationStateTime(AnimatorStateEventKeyMap.AnimatorStateBehaviourEventParameters parameters) 
    {
        ...
        Debug.Log("Current Multiplier " + parameters.animatorStateInfo.speedMultiplier + " stored length " + parameters.animatorStateInfo.length + " speed " + parameters.animatorStateInfo.speed);

        //Solve instant caseses that would result in division by zero
        if (seconds.Value == 0)
        {
            multiplierValue = float.PositiveInfinity;
        }
        else
        {
